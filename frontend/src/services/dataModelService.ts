...
const API_URL = 'https://localhost:3001/api/data-models';
...
export const dataModelService = {
  ...
  async getDatabaseSchema(connectionId: string): Promise<DatabaseSchemaResponse> {
    try {
      const authHeaders = await getAuthHeaders();
      const response = await axios.get(`https://localhost:3001/api/connections/${connectionId}/schema`, { headers: authHeaders });
      return response.data.data;
    } catch (error) {
  ...
  async getTableColumns(connectionId: string, schema: string, tableName: string): Promise<TableColumnsResponse> {
    try {
      ...
      const response = await axios.get(
        `https://localhost:3001/api/connections/${connectionId}/schema/${schema}/tables/${tableName}/columns`, 
        { headers: authHeaders }
      );
      return response.data.data;
    } catch (error) {
