...
// const API_URL = 'https://localhost:3000/api/connections';
...
export const databaseConnectionService = {
  ...
  async getAllConnections(): Promise<DatabaseConnection[]> {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/connections`, { headers: authHeaders });
  ...
  async getConnectionById(id: string): Promise<DatabaseConnection> {
    try {
      const authHeaders = await getAuthHeaders();
      const response = await axios.get(`${API_BASE_URL}/api/connections/${id}`, { headers: authHeaders });
      return response.data.data;
    } catch (error) {
  ...
  async createConnection(connectionData: CreateConnectionDto): Promise<DatabaseConnection> {
    try {
      const authHeaders = await getAuthHeaders();
      const response = await axios.post(`${API_BASE_URL}/api/connections`, connectionData, { headers: authHeaders });
      return response.data.data;
    } catch (error) {
  ...
  async updateConnection(id: string, connectionData: Partial<CreateConnectionDto>): Promise<DatabaseConnection> {
    try {
      const authHeaders = await getAuthHeaders();
      const response = await axios.put(`${API_BASE_URL}/api/connections/${id}`, connectionData, { headers: authHeaders });
      return response.data.data;
    } catch (error) {
  ...
  async deleteConnection(id: string): Promise<void> {
    try {
      const authHeaders = await getAuthHeaders();
      await axios.delete(`${API_BASE_URL}/api/connections/${id}`, { headers: authHeaders });
    } catch (error) {
  ...
  async testConnection(id: string): Promise<{ status: 'active' | 'inactive', lastTestedAt: string }> {
    try {
      const authHeaders = await getAuthHeaders();
      const response = await axios.post(`${API_BASE_URL}/api/connections/${id}/test`, {}, { headers: authHeaders });
      return response.data.data;
    } catch (error) {
  ...
  async getDatabaseSchema(connectionId: string): Promise<{ tables: DatabaseTable[] }> {
    try {
      const authHeaders = await getAuthHeaders();
      const response = await axios.get(`${API_BASE_URL}/api/connections/${connectionId}/schema`, { headers: authHeaders });
      return response.data.data;
    } catch (error) {
  ...
  async getConnectionsByProjectId(projectId: string): Promise<DatabaseConnection[]> {
    try {
      const authHeaders = await getAuthHeaders();
      const response = await axios.get(`${API_BASE_URL}/api/connections/by-project/${projectId}`, { headers: authHeaders });
      return response.data.data;
    } catch (error) {
  ...
  async executeQuery(connectionId: string, query: string): Promise<any[]> {
    try {
      const authHeaders = await getAuthHeaders();
      const response = await axios.post(`${API_BASE_URL}/api/connections/${connectionId}/query`, { query }, { headers: authHeaders });
      return response.data.data;
    } catch (error) {
